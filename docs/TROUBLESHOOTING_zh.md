...
## LM Studio 连接被拒绝 / WinError 10061

如果看到 `WinError 10061`、`Connection refused`、`Max retries exceeded` 或 `HTTPConnectionPool(host='127.0.0.1', port=1234)`，说明 GUI/CLI 无法连接 LM Studio。

- 打开 LM Studio。
- 启动 Local Server。
- 确认 Base URL：`http://127.0.0.1:1234/v1`。
- 在 GUI 点击**测试 LM Studio 连接**。
- 如有需要检查防火墙。

关闭 GUI 时会先尝试停止子进程树。若仍有残留，请在任务管理器结束 `python.exe` / `GrokStoryHandoff.exe`。
